...
using cminor::codedom::IdExpr;

class CodeExpandingVisitor : public cminor::codedom::Visitor
{
public:
    ...
    ParsingObject(name_, grammar_->GetScope()), linkedRuleName(linkedRuleName_), grammar(grammar_)
{
    ...
    if (prefix == U"stdlib")
    {
        linkedRuleName = U"cminor.parsing.stdlib." + GetCommonName(linkedRuleName);
    }
}
