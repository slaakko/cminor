...
#pragma warning(default: 4244)
    ...
    hppFormatter.WriteLine("#include <cminor/parsing/Grammar.hpp>");
    hppFormatter.WriteLine("#include <cminor/parsing/Keyword.hpp>");
    if (hppFileName != "StdLib.hpp")
    {
        cppFormatter.WriteLine("#include \"" + hppFileName + "\"");
    }
    cppFormatter.WriteLine("#include <cminor/parsing/Action.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/Rule.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/ParsingDomain.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/Primitive.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/Composite.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/Nonterminal.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/Exception.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/StdLib.hpp>");
    cppFormatter.WriteLine("#include <cminor/parsing/XmlLog.hpp>");
    cppFormatter.WriteLine("#include <cminor/util/Unicode.hpp>");
    ...
    for (NamespaceContentMapIt i = nsContent.begin(); i != e; ++i)
    {
        ...
        if (!content.UsingObjects().empty())
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                cminor::codedom::UsingObject* subject = usingObject->Subject();
        ...
        cppFormatter.WriteLine("using namespace cminor::util;");
        cppFormatter.WriteLine("using namespace cminor::unicode;");
        cppFormatter.WriteLine();
