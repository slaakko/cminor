...
namespace cminor { namespace unicode {
...
std::string MakeCanonicalPropertyName(const std::string& s)
{
    ...
    return cminor::util::ToLower(propertyName);
}
...
{
    static_cast<uint8_t>('C'), static_cast<uint8_t>('M'), static_cast<uint8_t>('N'), static_cast<uint8_t>('R'),
    static_cast<uint8_t>('U'), static_cast<uint8_t>('C'), static_cast<uint8_t>('D'), current_cminor_ucd_version
};

std::string CminorRoot()
{
    std::string cminorRoot;
    const char* cminorRootEnv = getenv("CMINOR_ROOT");
    if (cminorRootEnv)
    {
        cminorRoot = cminorRootEnv;
    }
    if (cminorRoot.empty())
    {
        throw UnicodeException("please set 'CMINOR_ROOT' environment variable to contain /path/to/cminor directory.");
    }
    return cminorRoot;
}

std::string CminorUcdFilePath()
{
    return (boost::filesystem::path(CminorRoot()) / boost::filesystem::path("unicode") / boost::filesystem::path("cminor_ucd.bin")).generic_string();
}
...
{
    std::string ucdFilePath = CminorUcdFilePath();
...
void CharacterTable::ReadHeader(BinaryReader& reader)
{
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        {
            throw UnicodeException("invalid cminor_ucd.bin header magic: 'CMNRUCD' expected");
        }
    }
    if (magic[7] != headerMagic[7])
    {
        throw UnicodeException("invalid cminor_ucd.bin version: version " + std::string(1, headerMagic[7]) + " expected, version " + std::string(1, magic[7]) + " read");
    }
